...
# We don't use mcp.server.fastmcp anymore as requested, just regular python functions
...
def init_tables():
    conn = get_connection()
    cur = conn.cursor()
    # The reference used dq_rules_proposed, but let's use dq_rules as requested if that's what the schema is
    cur.execute("""
...
def get_profile(table_name: str, column_name: str):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        SELECT
